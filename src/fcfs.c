...
/* ------------------------------------------------------------------
 * Algorithm:
 *   1. Sort processes by arrival_time (lower PID wins ties)
 *   2. Walk through the sorted list.
 *      If the CPU is idle (current_time < arrival), insert an IDLE
 *      Gantt entry and jump the clock forward.
 *   3. Run each process to completion. Record start_time, then compute
 *      finish_time, turnaround_time, and waiting_time inline.
 *
